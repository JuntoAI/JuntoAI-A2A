...
class TestStatsEndpointLogic:
    ...
    def test_returns_valid_stats_response_shape(self):
        ...
        sessions = [
            ...
            _make_session_doc(
                ...
                agent_calls=[{"model_id": "gemini-3.5-flash", "input_tokens": 100, "output_tokens": 50, "latency_ms": 400}],
            ),
        ]
    ...
    def test_model_tokens_and_performance(self):
        ...
        sessions = [
            _make_session_doc(
                "s1", agent_calls=[
                    {"model_id": "gemini-3.5-flash", "input_tokens": 100, "output_tokens": 200, "latency_ms": 500},
                    {"model_id": "claude-sonnet-4", "input_tokens": 150, "output_tokens": 250, "latency_ms": 800},
                ],
            ),
            _make_session_doc(
                "s2", agent_calls=[
                    {"model_id": "gemini-3.5-flash", "input_tokens": 80, "output_tokens": 120, "latency_ms": 450},
                ],
            ),
        ]
        ...
        assert "gemini-3.5-flash" in token_map
        assert "claude-sonnet-4" in token_map
        assert token_map["gemini-3.5-flash"].tokens_today == 500  # (100+200) + (80+120)
        assert token_map["claude-sonnet-4"].tokens_today == 400  # 150+250

        perf_map = {m.model_id: m for m in result.model_performance}
        assert perf_map["gemini-3.5-flash"].avg_response_time_today == pytest.approx(475.0)  # (500+450)/2
