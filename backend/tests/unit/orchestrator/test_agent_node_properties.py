...
st_model_id = st.sampled_from(["gemini-3.5-flash", "gemini-3.5-flash", "claude-3-5-sonnet-v2", "claude-sonnet-4-6"])
...
def st_negotiator_state(draw: st.DrawFn) -> tuple[NegotiationState, str, NegotiatorOutput]:
    ...
    agent_states = {
        role: {"role": role, "name": "A", "agent_type": "negotiator", "model_id": "gemini-3.5-flash", "last_proposed_price": 0.0, "warning_count": 0},
        other_role: {"role": other_role, "name": "B", "agent_type": "negotiator", "model_id": "gemini-3.5-flash", "last_proposed_price": 0.0, "warning_count": 0},
    }
...
def st_observer_state(draw: st.DrawFn) -> tuple[NegotiationState, str, ObserverOutput]:
    ...
    role = draw(st_role)
    agent_states = {
        role: {"role": role, "name": "O", "agent_type": "observer", "model_id": "gemini-3.5-flash", "last_proposed_price": 0.0, "warning_count": 0},
    }
    state = NegotiationState(
        session_id="s", scenario_id="s", turn_count=0,
        max_turns=15, current_speaker=role, deal_status="Negotiating",
...
def st_full_cycle_state(draw: st.DrawFn) -> NegotiationState:
    ...
    for r in negotiator_roles:
        agent_states[r] = {
            ...
            "model_id": "gemini-3.5-flash", "last_proposed_price": 0.0, "warning_count": 0,
        }
...
def st_memory_enabled_prompt_scenario(
    ...
    scenario_config: dict[str, Any] = {
        ...
        "agents": [
            agent_config,
            {"role": other_role, "name": "Seller", "type": "negotiator", "model_id": "gemini-3.5-flash"},
        ],
...
def st_memory_extractor_update_scenario(
    ...
    scenario_config: dict[str, Any] = {
        "id": "test",
        "agents": [
            {"role": role, "name": "Agent A", "type": "negotiator", "model_id": "gemini-3.5-flash"},
            {"role": other_role, "name": "Agent B", "type": "negotiator", "model_id": "gemini-3.5-flash"},
        ],
        "negotiation_params": {"max_turns": 15},
    }
