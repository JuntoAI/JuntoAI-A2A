...
def _make_allowed_models(
    ...
    if probe_results is None:
        probe_results = [
            ProbeResult("gemini-3.5-flash", "gemini", True, None, 234.5),
...
class TestAdminModelsResponseShape:
    ...
    async def test_response_contains_summary_counts(self, app_instance):
        ...
        probes = [
            ProbeResult("gemini-3.5-flash", "gemini", True, None, 200.0),
            ProbeResult("gemini-3.5-flash", "gemini", True, None, 350.0),
    ...
    async def test_response_model_entries_shape(self, app_instance):
        ...
        available_model = next(m for m in models if m["model_id"] == "gemini-3.5-flash")
    ...
    async def test_all_models_unavailable_degraded(self, app_instance):
        ...
        probes = [
            ProbeResult("gemini-3.5-flash", "gemini", False, "ConnectionError", 50.0),
