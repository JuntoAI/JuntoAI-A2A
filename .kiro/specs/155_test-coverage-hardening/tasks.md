...
- [ ] 1. Test infrastructure and configuration
  - [ ] 1.1 Update `backend/pytest.ini` with markers
    - Add `markers` section with `unit`, `integration`, `property`, `slow`
    - Keep existing `asyncio_mode`, `addopts`, `testpaths`
    - _Requirements: 1.3_

  - [ ] 1.2 Extend `backend/tests/conftest.py` with new shared fixtures
  ...
  - [ ] 1.3 Verify test suite runs under 120s
...
- [ ] 2. SSE event formatting tests (highest priority)
  - [ ] 2.1 Create `backend/tests/unit/test_snapshot_to_events.py`
  ...
  - [ ] 2.2 Create `backend/tests/unit/test_participant_summaries.py`
  ...
  - [ ] 2.3 Create SSE format property test
...
- [ ] 4. Model mapping and pure logic tests
  - [ ] 4.1 Create `backend/tests/unit/test_model_mapping.py`
  ...
  - [ ] 4.2 Add model mapping property test
...
- [ ] 5. Database client tests
  - [ ] 5.1 Create `backend/tests/unit/test_sqlite_client.py` (new or extend existing)
  ...
  - [ ] 5.2 Create `backend/tests/unit/test_firestore_client.py`
  ...
  - [ ] 5.3 Create `backend/tests/unit/test_profile_client.py`
  ...
  - [ ] 5.4 Add SQLite round-trip property test
...
- [ ] 6. Checkpoint — Database and model mapping tests
  - Run coverage on `app/db/`, `app/orchestrator/model_mapping.py`. Verify coverage increased.

- [ ] 7. Middleware tests
  - [ ] 7.1 Create `backend/tests/unit/test_event_buffer.py`
  ...
  - [ ] 7.2 Extend `backend/tests/unit/test_sse_limiter.py`
  ...
  - [ ] 7.3 Add event buffer replay property test
...
- [ ] 8. Auth service tests
  - [ ] 8.1 Create `backend/tests/unit/test_auth_service.py`
  ...
  - [ ] 8.2 Add password hash property test
...
- [ ] 9. Checkpoint — Middleware and auth tests
  - Run coverage on `app/middleware/`, `app/services/`. Verify coverage increased.

- [ ] 10. Negotiation router integration tests
  - [ ] 10.1 Create `backend/tests/integration/test_negotiation_start.py`
  ...
  - [ ] 10.2 Extend `backend/tests/integration/test_stream.py`
  ...
  - [ ] 10.3 Extend `backend/tests/integration/test_auth_endpoints.py`
  ...
  - [ ] 10.4 Extend `backend/tests/integration/test_profile_endpoints.py`
...
- [ ] 11. Evaluator and orchestrator gap tests
  - [ ] 11.1 Extend `backend/tests/unit/orchestrator/test_evaluator.py`
  ...
  - [ ] 11.2 Extend `backend/tests/unit/orchestrator/test_evaluator_prompts.py`
  ...
  - [ ] 11.3 Extend `backend/tests/unit/orchestrator/test_confirmation_node.py`
  ...
  - [ ] 11.4 Extend `backend/tests/unit/orchestrator/test_milestone_generator.py`
    - Add tests for milestone generation at different negotiation stages
    - Add tests for milestone summary formatting
    - _Requirements: 10.1_

- [ ] 12. Scenario module gap tests
  - [ ] 12.1 Extend `backend/tests/unit/test_toggle_injector.py`
  ...
  - [ ] 12.2 Extend `backend/tests/unit/test_pretty_printer.py`
...
- [ ] 14. Frontend API client tests
  ...
  - [ ] 14.2 Create `frontend/__tests__/lib/profile.test.ts`
    - Mock `global.fetch` with `vi.fn()`
...
- [ ] 15. Frontend component tests
  - [ ] 15.1 Create `frontend/__tests__/components/WaitlistForm.test.tsx`
  ...
  - [ ] 15.2 Create `frontend/__tests__/components/TokenDisplay.test.tsx`
    - Render with token count (e.g., 50) → displays "50"
    - Render with 0 tokens → displays "0"
    - Render with undefined/null → handles gracefully
    - _Requirements: 9.2_

  - [ ] 15.3 Create `frontend/__tests__/components/StartNegotiationButton.test.tsx`
