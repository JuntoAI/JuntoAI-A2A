...
- [ ] 1. Extend state, models, and output schemas
  - [ ] 1.1 Add `ConfirmationOutput`, `EvaluationInterview`, and `EvaluationReport` to `backend/app/orchestrator/outputs.py`
  ...
  - [ ] 1.2 Add `closure_status` and `confirmation_pending` to `NegotiationState` TypedDict in `backend/app/orchestrator/state.py`
    - `closure_status: str` (default `""`)
    - `confirmation_pending: list[str]` (default `[]`)
    - Update `create_initial_state()` to initialize both fields
    - _Requirements: 4.1, 4.2_

  - [ ] 1.3 Extend `NegotiationStateModel` in `backend/app/models/negotiation.py`
  ...
  - [ ] 1.4 Add `EvaluatorConfig` model and `evaluator_config` field to `ArenaScenario` in `backend/app/scenarios/models.py`
  ...
  - [ ] 1.5 Add `EvaluationInterviewEvent` and `EvaluationCompleteEvent` to `backend/app/models/events.py`
  ...
  - [ ] 1.6 Update converters in `backend/app/orchestrator/converters.py`
    - Map `closure_status` and `confirmation_pending` in both `to_pydantic()` and `from_pydantic()`
    - _Requirements: 4.1, 4.2, 4.3_

  - [ ]* 1.7 Write property test for model round-trip serialization (Property 5)
...
- [ ] 3. Implement confirmation node and dispatcher modifications
  - [ ] 3.1 Create `backend/app/orchestrator/confirmation_node.py`
  ...
  - [ ] 3.2 Modify `_dispatcher()` in `backend/app/orchestrator/graph.py`
  ...
  - [ ] 3.3 Modify `_route_dispatcher()` in `backend/app/orchestrator/graph.py`
    - When `deal_status == "Confirming"`, return `"confirmation"` node name
    - _Requirements: 1.1, 4.4_

  - [ ] 3.4 Modify `build_graph()` in `backend/app/orchestrator/graph.py`
  ...
  - [ ]* 3.5 Write property test: convergence triggers Confirming, never Agreed directly (Property 1)
  ...
  - [ ]* 3.6 Write property test: confirmation_pending contains exactly negotiator roles (Property 2)
  ...
  - [ ]* 3.7 Write property test: confirmation resolution is deterministic and correct (Property 3)
  ...
  - [ ]* 3.8 Write property test: confirmation node appends correct history entries (Property 4)
  ...
  - [ ]* 3.9 Write property test: confirmation prompt contains converged terms (Property 9)
  ...
  - [ ]* 3.10 Write unit tests for confirmation node
...
- [ ] 5. Implement evaluator agent (standalone, outside graph)
  - [ ] 5.1 Create `backend/app/orchestrator/evaluator_prompts.py`
  ...
  - [ ] 5.2 Create `backend/app/orchestrator/evaluator.py`
  ...
  - [ ]* 5.3 Write property test: evaluator interviews exactly N negotiators (Property 6)
  ...
  - [ ]* 5.4 Write property test: interview isolation — no cross-contamination (Property 7)
  ...
  - [ ]* 5.5 Write property test: interview prompt contains required context (Property 8)
  ...
  - [ ]* 5.6 Write property test: default evaluator model resolution (Property 10)
  ...
  - [ ]* 5.7 Write property test: scoring prompt includes objective deal metrics (Property 11)
  ...
  - [ ]* 5.8 Write unit tests for evaluator and evaluator prompts
...
- [ ] 7. Modify streaming endpoint and snapshot-to-events
  - [ ] 7.1 Update `_snapshot_to_events()` in `backend/app/routers/negotiation.py`
  ...
  - [ ] 7.2 Update `event_stream()` generator in `backend/app/routers/negotiation.py`
  ...
  - [ ]* 7.3 Write unit tests for SSE event models
...
- [ ] 9. Frontend: OutcomeReceipt evaluation display
  - [ ] 9.1 Extend `OutcomeReceipt` component in `frontend/components/glassbox/OutcomeReceipt.tsx`
  ...
  - [ ]* 9.2 Write frontend tests for OutcomeReceipt evaluation display
...
- [ ] 10. Integration tests
  - [ ]* 10.1 Write integration test: full confirmation flow
  ...
  - [ ]* 10.2 Write integration test: evaluation event ordering and graceful degradation
