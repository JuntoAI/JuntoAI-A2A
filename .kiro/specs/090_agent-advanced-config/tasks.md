...
- [ ] 1. Extend backend state and data models
  - [ ] 1.1 Add `custom_prompts` and `model_overrides` fields to `NegotiationState` TypedDict in `backend/app/orchestrator/state.py`
  ...
  - [ ] 1.2 Add `custom_prompts` and `model_overrides` fields to `NegotiationStateModel` in `backend/app/models/negotiation.py`
  ...
  - [ ] 1.3 Update `StartNegotiationRequest` in `backend/app/routers/negotiation.py`
    - Add `custom_prompts: dict[str, str] = Field(default_factory=dict)` and `model_overrides: dict[str, str] = Field(default_factory=dict)`
  ...
  - [ ]* 1.4 Write property tests for `StartNegotiationRequest` round-trip serialization (P1) and custom prompt length validation (P2)
  ...
  - [ ]* 1.5 Write property test for state persistence round-trip (P10)
...
- [ ] 2. Create GET /api/v1/models endpoint
  - [ ] 2.1 Create `backend/app/routers/models.py` with a `GET /models` endpoint
  ...
  - [ ] 2.2 Register the models router in `backend/app/main.py`
    - Import and include `models_router` in the `api_router` with `api_router.include_router(models_router)`
    - _Requirements: 9.1_

  - [ ]* 2.3 Write property test for available models endpoint (P7)
...
- [ ] 3. Update negotiation router to handle custom_prompts and model_overrides
  - [ ] 3.1 Update `start_negotiation` endpoint in `backend/app/routers/negotiation.py`
  ...
  - [ ] 3.2 Update `stream_negotiation` endpoint in `backend/app/routers/negotiation.py`
    - Pass `custom_prompts` and `model_overrides` from the loaded `NegotiationStateModel` state to `create_initial_state`
    - _Requirements: 13.2, 13.4_

  - [ ]* 3.3 Write property tests for invalid model rejection (P3) and unknown role key filtering (P4)
...
- [ ] 4. Checkpoint — Backend models and API
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Update orchestrator agent_node for custom prompt injection and model override routing
  - [ ] 5.1 Update `_build_prompt` in `backend/app/orchestrator/agent_node.py` for custom prompt injection
  ...
  - [ ] 5.2 Update `create_agent_node._node` in `backend/app/orchestrator/agent_node.py` for model override routing
  ...
  - [ ]* 5.3 Write property test for custom prompt injection (P5)
  ...
  - [ ]* 5.4 Write property test for model override routing (P6)
...
- [ ] 7. Add frontend API client functions
  - [ ] 7.1 Add `fetchAvailableModels` function and `ModelInfo` interface to `frontend/lib/api.ts`
  ...
  - [ ] 7.2 Update `startNegotiation` function signature in `frontend/lib/api.ts`
  ...
  - [ ]* 7.3 Write property test for request payload filtering (P9)
...
- [ ] 8. Create AdvancedConfigModal component
  - [ ] 8.1 Create `frontend/components/arena/AdvancedConfigModal.tsx`
  ...
  - [ ]* 8.2 Write property test for custom prompt character limit enforcement (P8)
  ...
  - [ ]* 8.3 Write property test for model selector default ordering and labeling (P12)
  ...
  - [ ]* 8.4 Write property test for config save-and-reload round-trip (P13)
...
- [ ] 9. Update AgentCard with Advanced Config button and indicators
  - [ ] 9.1 Update `AgentCard` component in `frontend/components/arena/AgentCard.tsx`
  ...
  - [ ]* 9.2 Write property test for agent card visual indicators (P11)
...
- [ ] 10. Update Arena page state management and wiring
  - [ ] 10.1 Update `ArenaPageContent` in `frontend/app/(protected)/arena/page.tsx`
...
- [ ] 12. Unit tests for edge cases
  - [ ]* 12.1 Write unit tests for AdvancedConfigModal interactions
  ...
  - [ ]* 12.2 Write unit tests for Arena page state management
