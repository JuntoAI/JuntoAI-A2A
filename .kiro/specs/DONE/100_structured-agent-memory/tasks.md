...
- [x] 1. Add AgentMemory model and update state infrastructure
  - [x] 1.1 Create `AgentMemory` Pydantic V2 model in `backend/app/orchestrator/outputs.py`
  ...
  - [x] 1.2 Write property test for AgentMemory serialization round-trip
  ...
  - [x] 1.3 Write property test for AgentMemory JSON serializability
  ...
  - [x] 1.4 Add `structured_memory_enabled` and `agent_memories` to `NegotiationState` TypedDict in `backend/app/orchestrator/state.py`
  ...
  - [x] 1.5 Write property test for `create_initial_state` memory initialization
  ...
  - [x] 1.6 Add `structured_memory_enabled` and `agent_memories` to `NegotiationStateModel` in `backend/app/models/negotiation.py`
...
- [x] 3. Update converters and API layer
  - [x] 3.1 Update `to_pydantic` and `from_pydantic` in `backend/app/orchestrator/converters.py`
  ...
  - [x] 3.2 Write property test for full state round-trip with memory
  ...
  - [x] 3.3 Add `structured_memory_enabled` to `StartNegotiationRequest` in `backend/app/routers/negotiation.py`
...
- [x] 4. Implement memory-aware prompt building in `_build_prompt`
  - [x] 4.1 Modify `_build_prompt` in `backend/app/orchestrator/agent_node.py` for structured memory mode
  ...
  - [x] 4.2 Write property test for memory-enabled prompt format
  ...
  - [x] 4.3 Write property test for disabled memory prompt identity
...
- [x] 6. Implement memory extraction in `_update_state`
  - [x] 6.1 Add memory extraction logic to `_update_state` in `backend/app/orchestrator/agent_node.py`
  ...
  - [x] 6.2 Write property test for memory extractor updates
  ...
  - [x] 6.3 Write property test for opposing offer capture
  ...
  - [x] 6.4 Write property test for memory extraction skipped when disabled
...
- [x] 7. Update stall detector for structured memory
  - [x] 7.1 Modify `_get_prices_by_role` in `backend/app/orchestrator/stall_detector.py`
  ...
  - [x] 7.2 Write property test for stall detector equivalence
...
- [x] 9. Frontend: Advanced Options UI and API wiring
  - [x] 9.1 Add `structuredMemoryEnabled` parameter to `startNegotiation` in `frontend/lib/api.ts`
  ...
  - [x] 9.2 Add collapsible "Advanced Options" section to `frontend/app/(protected)/arena/page.tsx`
  ...
  - [x] 9.3 Write frontend tests for Advanced Options section
