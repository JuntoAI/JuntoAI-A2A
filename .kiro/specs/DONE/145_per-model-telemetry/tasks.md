...
- [x] 1. AgentCallRecord model and state plumbing
  - [x] 1.1 Add `AgentCallRecord` Pydantic model to `backend/app/orchestrator/outputs.py`
  ...
  - [x] 1.2 Add `agent_calls` field to `NegotiationState` TypedDict in `backend/app/orchestrator/state.py`
    - Add `agent_calls: Annotated[list[dict[str, Any]], add]` following the same append-on-merge pattern as `history`
    - _Requirements: 2.1_

  - [x] 1.3 Initialize `agent_calls=[]` in `create_initial_state()` in `backend/app/orchestrator/state.py`
    - Add `agent_calls=[]` to the returned `NegotiationState` dict
    - _Requirements: 2.2_

  - [x] 1.4 Add `agent_calls` field to `NegotiationStateModel` in `backend/app/models/negotiation.py`
    - Add `agent_calls: list[dict[str, Any]] = Field(default_factory=list)`
    - _Requirements: 2.3_

  - [x] 1.5 Update converters in `backend/app/orchestrator/converters.py`
  ...
  - [x] 1.6 Write property test: AgentCallRecord round-trip serialization (Property 1)
  ...
  - [x] 1.7 Write property test: Converter round-trip preserves agent_calls (Property 2)
  ...
  - [x] 1.8 Write unit tests for AgentCallRecord and state plumbing
...
- [x] 2. Checkpoint — State plumbing
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Agent node instrumentation
  - [x] 3.1 Add `_extract_tokens()` helper to `backend/app/orchestrator/agent_node.py`
  ...
  - [x] 3.2 Instrument the first `model.invoke()` call in `_node()` with telemetry
  ...
  - [x] 3.3 Instrument the retry `model.invoke()` call in `_node()` with telemetry
  ...
  - [x] 3.4 Include `agent_calls` in the state delta returned by `_node()`
  ...
  - [x] 3.5 Write property test: Token extraction correctness (Property 3)
  ...
  - [x] 3.6 Write unit tests for agent node telemetry instrumentation
...
- [x] 4. Final checkpoint — All telemetry complete
  - Ensure all tests pass, ask the user if questions arise.
