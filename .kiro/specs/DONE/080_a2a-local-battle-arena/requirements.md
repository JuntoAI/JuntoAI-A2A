...
The JuntoAI A2A Local Battle Arena enables the existing cloud MVP to run entirely on a developer's machine with zero GCP dependencies and zero API key configuration. Developers clone the repo, run `docker compose up`, and get the full Arena (FastAPI backend + Next.js frontend + Ollama LLM) on localhost — a working negotiation out of the box with no `.env` edits required.

Ollama runs as a Docker Compose sidecar service, auto-pulls a default model (e.g., `llama3.1`) on startup, and serves as the default `LLM_PROVIDER`. All scenario `model_id` values (e.g., `gemini-2.5-flash`, `gemini-2.5-pro`) are transparently mapped to the local Ollama model. Developers who prefer OpenAI, Anthropic, or other providers can override via `.env`.

The cloud deployment continues working exactly as before — this is purely additive.

This spec covers six layers: database (SQLite fallback), LLM routing (LiteLLM), Ollama integration (zero-config default), auth bypass, Docker Compose infrastructure, and environment configuration. It does NOT cover custom agent plugins or the global leaderboard (post-MVP).
...
- **Ollama_Service**: The Docker Compose sidecar service running the `ollama/ollama` image that provides a local LLM inference server accessible at `http://ollama:11434` from within the Docker network.
...
- **LLM_Provider**: An environment variable (`LLM_PROVIDER`) that selects the LLM backend for local mode. Defaults to `ollama` for zero-config operation. Supported values: `ollama`, `openai`, `anthropic`.
...
- **Docker_Compose_Stack**: The `docker-compose.yml` file at the monorepo root that launches the Backend, Frontend, and Ollama_Service containers together on localhost.
...
- **OLLAMA_BASE_URL**: The environment variable pointing the Backend to the Ollama API endpoint. Defaults to `http://ollama:11434` inside Docker Compose, overridable for host-native Ollama installs (e.g., `http://localhost:11434`).
- **OLLAMA_MODEL**: The environment variable specifying which Ollama model to pull and use. Defaults to `llama3.1`.
...
4. WHEN Run_Mode is `local`, THE LiteLLM_Router SHALL read the `LLM_PROVIDER` environment variable and map scenario `model_id` values to provider-specific model strings (e.g., `gemini-2.5-flash` → `ollama/llama3.1` when `LLM_PROVIDER=ollama`).
5. WHEN Run_Mode is `local` and `LLM_PROVIDER` is not `ollama` and the required API key is missing or invalid, THE LiteLLM_Router SHALL raise `ModelNotAvailableError` with a descriptive message naming the missing key.
...
7. WHEN Run_Mode is `local` and `LLM_PROVIDER` is `ollama`, THE LiteLLM_Router SHALL configure the Ollama base URL from the `OLLAMA_BASE_URL` environment variable (default: `http://ollama:11434`).
...
1. THE Backend SHALL define a default model mapping in `backend/app/orchestrator/model_mapping.py` that maps each scenario `model_id` to a default local model string per provider. The mapping SHALL cover all `model_id` values used in shipped scenario configs: `gemini-2.5-flash` and `gemini-2.5-pro`.
2. THE default Ollama mapping SHALL map all scenario `model_id` values to the model specified by the `OLLAMA_MODEL` environment variable (default: `llama3.1`), since local Ollama typically runs a single model.
3. WHERE a developer sets the `LLM_MODEL_OVERRIDE` environment variable, THE LiteLLM_Router SHALL use that single model for all agents regardless of scenario `model_id` values.
4. THE model mapping SHALL be overridable via a `MODEL_MAP` environment variable containing a JSON string (e.g., `{"gemini-2.5-flash": "gpt-4o-mini", "gemini-2.5-pro": "gpt-4o"}`).
5. IF a scenario `model_id` has no mapping for the current provider and no override is set, THEN THE LiteLLM_Router SHALL fall back to the provider's default model and log a warning.
6. THE default model mapping for the `openai` provider SHALL map `gemini-2.5-flash` to `gpt-4o-mini` and `gemini-2.5-pro` to `gpt-4o`.
7. THE default model mapping for the `anthropic` provider SHALL map `gemini-2.5-flash` to `claude-3-5-haiku-20241022` and `gemini-2.5-pro` to `claude-sonnet-4-20250514`.
...
2. THE `backend/app/config.py` Settings class SHALL include fields: `SQLITE_DB_PATH` (default: `data/juntoai.db`), `LLM_PROVIDER` (default: `ollama`), `LLM_MODEL_OVERRIDE` (default: empty string), `MODEL_MAP` (default: empty string), `OLLAMA_BASE_URL` (default: `http://ollama:11434`), `OLLAMA_MODEL` (default: `llama3.1`).
...
**User Story:** As a developer, I want a single `docker compose up` command to launch the full Arena (backend + frontend + LLM) on localhost, so that setup requires only Docker — no API keys, no `.env` edits for a first test.

#### Acceptance Criteria

1. THE monorepo root SHALL contain a `docker-compose.yml` file defining three services: `backend` (FastAPI on port 8000), `frontend` (Next.js on port 3000), and `ollama` (Ollama LLM server on port 11434).
2. WHEN `docker compose up` is executed, THE Docker_Compose_Stack SHALL build and start all containers with `RUN_MODE=local` and `LLM_PROVIDER=ollama` set by default.
...
8. THE `ollama` service SHALL use the `ollama/ollama` Docker image and expose port 11434.
9. THE `ollama` service SHALL mount a named volume (`ollama-data`) for model storage so that downloaded models persist across container restarts.
10. THE `backend` service SHALL depend on the `ollama` service so that the LLM server is available before the backend starts accepting requests.
11. THE `backend` service SHALL set `OLLAMA_BASE_URL=http://ollama:11434` as a default environment variable so the backend connects to the Ollama sidecar via the Docker network.
...
1. THE monorepo root SHALL contain a `.env.example` file with all configurable environment variables, grouped by category (Run Mode, LLM Provider, Ollama, Database, Frontend).
...
4. THE `.env.example` file SHALL set `RUN_MODE=local` and `LLM_PROVIDER=ollama` as the defaults, reflecting the zero-config experience.
5. THE `.env.example` file SHALL be listed in `.gitignore` as a pattern exception (`.env` is ignored, `.env.example` is tracked).
6. WHEN a developer runs `docker compose up` without creating a `.env` file, THE Docker_Compose_Stack SHALL start successfully using Ollama as the default LLM provider with no additional configuration.
...
3. WHEN Run_Mode is `local`, THE Model_Router SHALL translate scenario `model_id` values (e.g., `gemini-2.5-flash`, `gemini-2.5-pro`) to the local provider's equivalent models transparently.
...
### Requirement 14: Ollama Docker Compose Service

**User Story:** As a developer, I want Ollama pre-integrated as a Docker Compose service so that I get a working LLM out of the box with `docker compose up` — no API keys, no env configuration needed for a first test.

#### Acceptance Criteria

1. THE Docker_Compose_Stack SHALL define an `ollama` service using the `ollama/ollama` Docker image.
2. THE `ollama` service SHALL expose port 11434 on the host for direct API access and debugging.
3. THE `ollama` service SHALL mount a named volume (`ollama-data`) at `/root/.ollama` so that downloaded models persist across container restarts and are not re-downloaded on every `docker compose up`.
4. THE Docker_Compose_Stack SHALL include an `ollama-pull` init service that runs `ollama pull ${OLLAMA_MODEL:-llama3.1}` after the `ollama` service is healthy, ensuring the default model is available before the backend starts processing requests.
5. THE `ollama` service SHALL include a health check that verifies the Ollama API is responding (e.g., `curl -f http://localhost:11434/api/tags`).
6. THE `backend` service SHALL depend on the `ollama-pull` service (with `condition: service_completed_successfully`) so that the model is guaranteed to be pulled before the backend starts.
7. WHEN `LLM_PROVIDER` is set to a value other than `ollama` (e.g., `openai`, `anthropic`), THE `ollama` and `ollama-pull` services SHALL still start but the Backend SHALL not route requests to them.
8. THE `ollama-pull` service SHALL use the same `ollama/ollama` image and connect to the `ollama` service via the Docker network to execute the pull command.

### Requirement 15: Ollama Model Mapping Transparency

**User Story:** As a developer, I want all scenario `model_id` values (like `gemini-2.5-flash`, `gemini-2.5-pro`) to be transparently mapped to my local Ollama model, so that every scenario works without editing JSON configs.

#### Acceptance Criteria

1. WHEN `LLM_PROVIDER` is `ollama`, THE model mapping SHALL map all scenario `model_id` values to `ollama/{OLLAMA_MODEL}` (e.g., `ollama/llama3.1`), using the `OLLAMA_MODEL` environment variable.
2. WHEN `LLM_PROVIDER` is `ollama`, THE LiteLLM_Router SHALL set the `api_base` parameter to the value of `OLLAMA_BASE_URL` (default: `http://ollama:11434`) when constructing the LLM client.
3. WHEN `LLM_PROVIDER` is `ollama`, THE LiteLLM_Router SHALL not require any API key environment variables (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.).
4. IF `LLM_PROVIDER` is `ollama` and the Ollama server is unreachable at `OLLAMA_BASE_URL`, THEN THE LiteLLM_Router SHALL raise `ModelNotAvailableError` with a message indicating the Ollama server URL and suggesting the developer check that the Ollama service is running.
5. WHEN a developer overrides `OLLAMA_MODEL` (e.g., `OLLAMA_MODEL=mistral`), THE model mapping SHALL use the overridden model for all scenario `model_id` values and THE `ollama-pull` service SHALL pull that model instead of the default.
