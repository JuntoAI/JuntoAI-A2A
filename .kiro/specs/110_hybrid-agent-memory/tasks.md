...
- [ ] 1. Extend NegotiationParams with sliding window and milestone configuration
  - [ ] 1.1 Add `sliding_window_size` and `milestone_interval` fields to `NegotiationParams` in `backend/app/scenarios/models.py`
  ...
  - [ ]* 1.2 Write property test for NegotiationParams backward compatibility
  ...
  - [ ]* 1.3 Write unit tests for NegotiationParams new fields
...
- [ ] 2. Extend NegotiationState and create_initial_state with milestone fields
  - [ ] 2.1 Add milestone fields to `NegotiationState` TypedDict in `backend/app/orchestrator/state.py`
  ...
  - [ ] 2.2 Update `create_initial_state` in `backend/app/orchestrator/state.py`
  ...
  - [ ]* 2.3 Write property test for create_initial_state milestone initialization
  ...
  - [ ]* 2.4 Write unit tests for NegotiationState milestone fields
...
- [ ] 3. Extend NegotiationStateModel for Firestore persistence
  - [ ] 3.1 Add milestone fields to `NegotiationStateModel` in `backend/app/models/negotiation.py`
  ...
  - [ ]* 3.2 Write property test for milestone summary serialization round-trip
  ...
  - [ ]* 3.3 Write unit tests for NegotiationStateModel backward compatibility
...
- [ ] 5. Update converters to pass through milestone fields
  - [ ] 5.1 Update `to_pydantic` and `from_pydantic` in `backend/app/orchestrator/converters.py`
  ...
  - [ ]* 5.2 Write unit tests for converter round-trip with milestone fields
...
- [ ] 6. Implement MilestoneGenerator module
  - [ ] 6.1 Create `backend/app/orchestrator/milestone_generator.py`
  ...
  - [ ]* 6.2 Write unit tests for MilestoneGenerator
...
- [ ] 7. Integrate milestone generation into dispatcher
  - [ ] 7.1 Modify dispatcher in `backend/app/orchestrator/graph.py`
  ...
  - [ ]* 7.2 Write unit tests for dispatcher milestone triggering
...
- [ ] 8. Update Agent Node prompt builder for configurable sliding window and milestone summaries
  - [ ] 8.1 Modify `_build_prompt` in `backend/app/orchestrator/agent_node.py`
  ...
  - [ ]* 8.2 Write property test for prompt token boundedness
  ...
  - [ ]* 8.3 Write unit tests for prompt builder milestone integration
...
- [ ] 10. Update StartNegotiationRequest and negotiation router
  - [ ] 10.1 Add `milestone_summaries_enabled` field to `StartNegotiationRequest` in `backend/app/routers/negotiation.py`
    - `milestone_summaries_enabled: bool = Field(default=False)`
    - _Requirements: 6.2, 6.5_

  - [ ] 10.2 Update `start_negotiation` and `stream_negotiation` in `backend/app/routers/negotiation.py`
  ...
  - [ ]* 10.3 Write unit tests for negotiation router milestone toggle
...
- [ ] 11. Add Milestone Summaries toggle to frontend
  - [ ] 11.1 Create `MilestoneSummariesToggle` component in `frontend/components/arena/`
  ...
  - [ ] 11.2 Wire toggle state logic in `frontend/app/(protected)/arena/page.tsx`
  ...
  - [ ]* 11.3 Write frontend tests for MilestoneSummariesToggle
...
- [ ] 12. Update frontend API client to pass milestone toggle
  - [ ] 12.1 Update `startNegotiation` in `frontend/lib/api.ts`
  ...
  - [ ] 12.2 Update `handleInitialize` in `frontend/app/(protected)/arena/page.tsx`
    - Pass `milestoneSummariesEnabled` state to `startNegotiation` call
    - _Requirements: 6.1_

  - [ ]* 12.3 Write frontend tests for API client milestone parameter
