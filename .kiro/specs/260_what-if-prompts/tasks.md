...
- [ ] 1.1 Add `target_agent_role: string` to `ToggleDefinition` interface in `frontend/lib/api.ts`
    - The backend already returns this field; the frontend was ignoring it
    - _Requirements: 1.3, 5.4_
- [ ] 1.2 Create `frontend/lib/whatIfPrompts.ts` with types and `generateWhatIfPrompts()` function
...
- [ ] 1.3 Export `buildDeepLinkUrl(scenarioId: string, toggleIds: string[]): string` in `frontend/lib/whatIfPrompts.ts`
...
- [ ] 2.1 Add new optional props to `OutcomeReceiptProps` in `frontend/components/glassbox/OutcomeReceipt.tsx`
    - Add `toggles?: ToggleDefinition[]`
    - Add `activeToggleIds?: string[]`
    - Add `agents?: AgentDefinition[]`
    - Import types from `@/lib/api`
    - _Requirements: 4.3_
- [ ] 2.2 Add What-If prompt generation and card rendering to OutcomeReceipt
...
- [ ] 3.1 Update session page URL to include active toggle IDs
    ...
    - _Requirements: 4.2_
- [ ] 3.2 Pass toggle definitions, active toggle IDs, and agents to OutcomeReceipt in session page
...
- [ ] 4.1 Parse `toggles` query parameter in Arena page and activate valid toggles
...
- [ ] 5.1 Write property test: Inactive toggle computation is correct (`frontend/__tests__/properties/whatIfPrompts.property.test.ts`) [PBT]
...
- [ ] 5.2 Write property test: Prompt text contains toggle label and resolved agent name [PBT]
    ...
    - _Requirements: 1.3, 5.4_
- [ ] 5.3 Write property test: All-active scenario produces baseline prompt [PBT]
...
- [ ] 5.4 Write property test: Output length never exceeds 3 [PBT]
    ...
    - _Requirements: 1.5_
- [ ] 5.5 Write property test: Role diversity maximization [PBT]
    ...
    - _Requirements: 1.6_
- [ ] 5.6 Write property test: Deal-status-specific prompt content [PBT]
...
- [ ] 5.7 Write property test: Deep-link URL round-trip [PBT]
...
- [ ] 6.1 Write unit tests for Prompt Generator (`frontend/__tests__/lib/whatIfPrompts.test.ts`)
...
- [ ] 6.2 Write/update unit tests for OutcomeReceipt component (`frontend/__tests__/components/glassbox/OutcomeReceipt.test.tsx`)
...
- [ ] 7.1 Create `buildAdviceDeepLinkUrl` utility function in `frontend/lib/whatIfPrompts.ts`
...
- [ ] 7.2 Add "Try This" button to each advice item in `OutcomeReceipt.tsx`
...
- [ ] 8.1 Add `customPrompts` query param parsing in Arena page (`frontend/app/(protected)/arena/page.tsx`)
...
- [ ] 9.1 Write property test: Custom prompts URL encoding round-trip (`frontend/__tests__/properties/adviceDeepLink.property.test.ts`) [PBT]
...
- [ ] 9.2 Write property test: Only valid roles are prefilled (`frontend/__tests__/properties/adviceDeepLink.property.test.ts`) [PBT]
...
- [ ] 10.1 Write unit tests for advice "Try This" button in OutcomeReceipt (`frontend/__tests__/components/glassbox/OutcomeReceipt.test.tsx`)
...
- [ ] 10.2 Write unit tests for Arena page `customPrompts` param parsing (`frontend/__tests__/pages/arena-advanced-config.test.tsx`)
