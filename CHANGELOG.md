...
## EspoCRM Contact Sync (Spec 360) — 2026-04-21
...
- `CrmSyncResult` Pydantic V2 model (`email`, `action: Literal["created","updated","skipped","error"]`, `detail: str | None`) added to `app/models/admin.py`
...
- `POST /api/v1/admin/users/{email}/sync-crm` endpoint: awaited sync, returns `CrmSyncResult` HTTP 200 for all action values, 404 if user not in waitlist, 503 in local mode via existing `verify_admin_session`
